...
- **`tagged-devices` is a hardcoded virtual user** (`hscontrol/types/users.go:36`, fixed ID `2147455555`). It owns every tagged node in API output but is **never persisted as a users row**. The actual `nodes.user_id` keeps the original owner; "tagged-devices" is rendered from an in-memory Go constant. Renaming requires forking Headscale — non-goal here. Don't propose a dashboard-only alias either: it would diverge the dashboard from `headscale nodes list`, the gRPC API, journalctl, and any other Tailscale-compatible tool.
